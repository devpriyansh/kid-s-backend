...
class ServiceBase {
  ...
  addError (attribute, errorMessage) {
    ...
    const isGlobalErrorType = attribute.endsWith('ErrorType')
    const formattedMessage = isGlobalErrorType ? errorMessage : `${_.startCase(attribute)} ${errorMessage}`

    if (!errors[attribute]) {
      _.extend(errors, { [attribute]: formattedMessage })
    } else {
      errors[attribute] = errors[attribute] instanceof Array ? errors[attribute] : [errors[attribute]]
      errors[attribute].push(formattedMessage)
    }
